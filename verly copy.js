...
class Verly {
  ...
  createCloth_perlin(posx, posy, w, h, segments, pinOffset) {
    ...
    // let xStride = w / segments;
    // let yStride = h / segments;
    let noiseScale = 0.5; // Adjust this to change the intensity of the noise

    let x, y;
    let perspectiveScale = 1; // Adjust this to change the intensity of the perspective effect

    for (y = 0; y < segments; ++y) {
      for (x = 0; x < segments; ++x) {
        let xStride = (w / segments) * (1 + p5Instance.noise(x / segments, y / segments) * noiseScale);
        let yStride = (h / segments) * (1 + p5Instance.noise(x / segments, y / segments) * noiseScale);
        let px = posx + x * xStride - w / 2 + xStride / 2;
        let py = posy + y * yStride - h / 2 + yStride / 2;
    
        // Add Perlin noise to the position
        px +=  p5Instance.noise(x / segments, y / segments) * noiseScale;
        py +=  p5Instance.noise(x / segments, y / segments) * noiseScale;
    
        // Add a perspective effect
        px += y * perspectiveScale;
...
// // Pin the bottom edge
// for (x = 0; x < segments; ++x) {
//   cloth.pin(segments * (segments - 1) + x);
// }

// // Pin the left edge
// for (y = 0; y < segments; ++y) {
//   cloth.pin(y * segments);
// }

// // Pin the right edge
// for (y = 0; y < segments; ++y) {
//   cloth.pin(y * segments + segments - 1);
// }
